...
void ChatLogic::LoadAnswerGraphFromFile(std::string filename)
{
            ...
            if (type != tokens.end())
            {
                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "NODE")
                    {
                        ...
                        // (7/9) Fixed Here (GraphNode *node) --> (std::unique_ptr<GraphNode> &node)
                    ...
                    if (type->second == "EDGE")
                    {
                        ...
                        if (parentToken != tokens.end() && childToken != tokens.end())
                        {
                            ...
                            // (7/9) Fixed Here (*childNode) --> ((*childNode).get())
                            // (7/9) Fixed Here (*parentNode) --> ((*parentNode).get())
                            // (7/9) Fixed Here (push_back(edge)) --> (push_back(std::move(edge)))
    ...
    // Task5:
    ChatBot _local_chatBot("../images/chatbot.png");
    _local_chatBot.SetChatLogicHandle(this);
    _local_chatBot.SetRootNode(rootNode);
    rootNode->MoveChatbotHere(std::move(_local_chatBot));
